...
def main():
    ...
    #ref_pose = pq_to_pose([0.3, 0.3, 0.3], [0.707, 0.707, 0.0, 0.0])
    listener = tf.TransformListener()
    listener.waitForTransform('/base_link', '/object_link', rospy.Time(), rospy.Duration(4.0))
    ...
    while not rospy.is_shutdown():
        ...
        try:
            (trans_object_in_base, rot_object_in_base) = listener.lookupTransform('/base_link', '/object_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue
        try:
            (trans_ee_in_gripper, rot_ee_in_gripper) = listener.lookupTransform('/gripper', '/ee_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue
            
        matrix_object_in_base = pose_to_matrix(pq_to_pose(trans_object_in_base, rot_object_in_base))
        matrix_ee_in_gripper = pose_to_matrix(pq_to_pose(trans_ee_in_gripper, rot_ee_in_gripper))
        ref_pose = matrix_to_pose(matrix_object_in_base.dot(matrix_ee_in_gripper))
